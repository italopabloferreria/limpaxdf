...
## Pasta oficial e estado atual

Use `C:/Users/italo/Programação/Limpax`. A cópia em `C:/Users/italo/Programação/01_PROJETOS/LIMPAX` está 14 commits atrás na comparação de 23/09/2026 e foi preservada. Não alternar edições entre as duas pastas.

O arquivo de regras é `AGENTS.md`; o resumo do andamento é `docs/AI_HANDOFF.md`; a tarefa corrente fica em `docs/CURRENT_TASK.md`.
...
- [Revisão geral e limpeza de 24/09/2026](docs/AUDIT_2026-09-24.md) — correções, arquivos removidos, testes e limitações atuais.
...
Homologação local: `npm run qa:supabase-adapter` e `npm run qa:supabase-session`. QA de navegador: instalar Chromium com `npx playwright install chromium`, depois executar `npm run qa:crm-network` e `npm run qa:customer-network`; essas duas suítes usam APIs sintéticas interceptadas.
